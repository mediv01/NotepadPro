...
LRESULT Notepad_plus::process(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_SETTINGCHANGE:
		{
			...
			// and don't change if NotepadPro is already in same mode as OS after changing OS mode
		...
		case NPPM_DESTROYSCINTILLAHANDLE:
		{
			...
			// Destroying allocated Scintilla makes NotepadPro crash
			...
			// So we do nothing here and let NotepadPro destroy allocated Scintilla while it exits
		...
		case NPPM_INTERNAL_WINDOWSSESSIONEXIT:
		{
			int answer = _nativeLangSpeaker.messageBox("WindowsSessionExit",
				...
				TEXT("Windows session is about to be terminated but you have some data unsaved. Do you want to exit NotepadPro now?"),
				TEXT("NotepadPro - Windows session exit"),
		...
		case WM_QUERYENDSESSION:
		{
			...
			if (::IsWindowEnabled(hwnd))
			{
				if (MainFileManager.getNbDirtyBuffers() > 0)
				{
					...
					// (but only for a non-critical OS restart/shutdown and while the NotepadPro backup mode is OFF)
					if (!isForcedShuttingDown && isFirstQueryEndSession && !nppParam.getNppGUI().isSnapshotMode())
					{
						// if NotepadPro has been minimized or invisible, we need to show it 1st
						if (::IsIconic(hwnd))
						{
			...
			else
			{
				...
				// here is the right place to unblock the modal-dlg blocking the main NotepadPro wnd, because then it will be too late
				...
				// otherwise the NotepadPro most probably crashes itself without any tidy-up

				string strLog = "Main NotepadPro wnd is disabled by (an active modal-dlg?):  ";
				...
				// re-test
				if (::IsWindowEnabled(hwnd))
					strLog += "  -> Main NotepadPro wnd has been successfully reenabled.";
			...
			// TODO: here is the last opportunity to call the following WINAPI in a possible future version of the NotepadPro
			// 
		...
		case WM_ENDSESSION:
		{
			...
			if (wParam == FALSE)
			{
				...
				// - it happens when either the NotepadPro returns FALSE to non-critical WM_QUERYENDSESSION or any other app with higher shutdown level
				//   than NotepadPro (app shuttdown order can be checked by the GetProcessShutdownParameters WINAPI)
			...
			else
			{
				...
				// do not return 0 here and continue to the NotepadPro standard WM_CLOSE code-part (no verbose GUI there this time!!!)
			}
		} // case WM_ENDSESSION:
		...
		case WM_CLOSE:
		{
			...
			else
			{
				...
				//Sends WM_DESTROY, NotepadPro will end
				::DestroyWindow(hwnd);
