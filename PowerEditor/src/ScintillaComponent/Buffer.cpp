...
void Buffer::updateTimeStamp()
{
	...
	if (res == -1 || res == 1)
	// (res == -1) => timeStampLive is later, it means the file has been modified outside of NotepadPro - usual case
	// 
	// (res == 1) => timeStampLive (get directly from the file on disk) is earlier than buffer's timestamp - unusual case
	//               It can happen when user copies a backup of editing file somewhere-else firstly, then modifies the editing file in NotepadPro and saves it.
	//               Now user copies the backup back to erase the modified editing file outside NotepadPro (via Explorer).
	{
		if (res == 1)
		{
...
bool Buffer::checkFileState() // returns true if the status has been changed (it can change into DOC_REGULAR too). false otherwise
{
	...
	else if (GetFileAttributesEx(_fullPathName.c_str(), GetFileExInfoStandard, &attributes) != 0)
	{
		...
		if (res == -1 || res == 1)
		// (res == -1) => attributes.ftLastWriteTime is later, it means the file has been modified outside of NotepadPro - usual case
		// 
...
For existing file (c:\tmp\foo.h)
	...
	- Close
	In the current session, NotepadPro
...
bool FileManager::loadFileData(Document doc, int64_t fileSize, const TCHAR * filename, char* data, Utf8_16_Read * unicodeConvertor, LoadedFileFormat& fileFormat)
{
	...
	if (bufferSizeRequested > INT_MAX)
	{
		...
		if (nppParam.archType() == IMAGE_FILE_MACHINE_I386)
		{
			pNativeSpeaker->messageBox("FileTooBigToOpen",
				...
				TEXT("File is too big to be opened by NotepadPro"),
	...
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
		...
		{
			case SC_STATUS_OK:
				// either the Scintilla doesn't catch this exception or the error is in the NotepadPro code, report the exception anyway
#if defined(__GNUC__)
